...
class FortranStates(CodeStateMachine):
    ...
    _block_ends = re.compile('(?:' + '|'.join(r'END\s*{0}'.format(_) for _ in FortranReader._blocks) + ')', re.I)
    _condition_ends = re.compile('(?:' + '|'.join(r'END\s*{0}'.format(_) for _ in FortranReader._conditions) + ')', re.I)
